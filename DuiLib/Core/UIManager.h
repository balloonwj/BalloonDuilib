...
public:
    //必须在OnCreate()等方法里面调用该方法将窗口与CPaintManagerUI关联起来
    ...
    /** 
     *@param nOpacity 不透明度设置，取值范围为0～255，
     *                设置成其他值则取消不透明度, win8以下系统只支持顶级窗口，
     *                win8系统支持顶级窗口和子窗口    
     **/
	void SetOpacity(BYTE nOpacity);

    /**
     *@return 窗口是否具有透明效果，具有返回false，反之返回true
     **/
	bool IsLayered();
...
private:
    ...
    //
    CControlUI* m_pRoot;                //实际上指的是xml中第二级节点，所以一般第一级节点为Window对象
    CControlUI* m_pFocus;
    CControlUI* m_pEventHover;
    CControlUI* m_pEventClick;          //产生鼠标事件的控件（在WM_LBUTTONDOWN消息处理中通过鼠标坐标在窗口区域找到）
    CControlUI* m_pEventKey;
    CControlUI* m_pLastToolTip;
    //
    //最近一次鼠标消息(WM_MOUSEMOVE/WM_LBUTTONDOWN/WM_LBUTTONDBLCLK
    ///WM_LBUTTONUP/WM_CONTEXTMENU/WM_MOUSEWHEEL)的鼠标坐标消息
    //相对于窗口客户区左上角（对于WM_CONTEXTMENU、WM_MOUSEWHEEL的屏幕坐标已经做过转换）
	...
	BYTE        m_nOpacity;
	bool        m_bLayered;         //窗口是否具有不透明效果，不具有为true，反之为false
	...
	bool        m_bIsPainting;      //是否正在绘制，在WM_PAINT绘制时，该变量为true
    ...
    CDuiStringPtrMap m_mOptionGroup;        //存放所有的单选控件组名
